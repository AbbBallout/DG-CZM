#ifndef FRICTION_EXTRINSIC_H
#define FRICTION_EXTRINSIC_H
...
namespace Extrinsice_friction
{
  ...
  template <int dim>
  class Extrinsice_friction
  {
  public:
    Extrinsice_friction(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  Extrinsice_friction<dim>::Extrinsice_friction(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
  ...
  template <int dim>
  void Extrinsice_friction<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin)
  {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::solve()
  {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::refine_mesh(const unsigned int cycle)
  {
                                   ...
                                   CopyData &copy_data)
    {
      ...
      if (cell->material_id() != ncell->material_id())
        for (unsigned int point = 0; point < n_q_points; ++point)
          if (quadrature_points_history[point].traction_init.norm() > sig_c / 2)
          {
      ...
      if (cell->refine_flag_set() && cell->level() == 2)
        cell->clear_refine_flag();

      if (ncell->refine_flag_set() && ncell->level() == 2)
    ...
    LA::MPI::Vector distributed_history_max_damage_field,
        ghosted_history_max_damage_field;
    distributed_history_max_damage_field.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_max_damage_field.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);

    LA::MPI::Vector
        distributed_history_traction_init_field0,
        ghosted_history_traction_init_field0,
        distributed_history_traction_init_field1,
        ghosted_history_traction_init_field1,
        distributed_history_law_g_field0,
        ghosted_history_law_g_field0,
        distributed_history_law_g_field1,
        ghosted_history_law_g_field1;

    distributed_history_traction_init_field0.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_traction_init_field0.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    distributed_history_traction_init_field1.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_traction_init_field1.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);

    distributed_history_law_g_field0.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_law_g_field0.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    distributed_history_law_g_field1.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_law_g_field1.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);

    Vector<double> local_history_max_damage_values_at_qpoints,
        local_history_max_damage_fe_values;
    local_history_max_damage_values_at_qpoints.reinit(face_quadrature->size());
    local_history_max_damage_fe_values.reinit(history_fe.dofs_per_cell);

    Vector<double>
        local_history_traction_init_values_at_qpoints0,
        local_history_traction_init_fe_values0,
        local_history_traction_init_values_at_qpoints1,
        local_history_traction_init_fe_values1,
        local_history_law_g_values_at_qpoints0,
        local_history_law_g_fe_values0,
        local_history_law_g_values_at_qpoints1,
        local_history_law_g_fe_values1;

    local_history_traction_init_values_at_qpoints0.reinit(face_quadrature->size());
    local_history_traction_init_fe_values0.reinit(history_fe.dofs_per_cell);
    local_history_traction_init_values_at_qpoints1.reinit(face_quadrature->size());
    local_history_traction_init_fe_values1.reinit(history_fe.dofs_per_cell);
    local_history_law_g_values_at_qpoints0.reinit(face_quadrature->size());
    local_history_law_g_fe_values0.reinit(history_fe.dofs_per_cell);
    local_history_law_g_values_at_qpoints1.reinit(face_quadrature->size());
    local_history_law_g_fe_values1.reinit(history_fe.dofs_per_cell);
    ...
    for (const auto cell : history_dof_handler.active_cell_iterators())
      if (cell->is_locally_owned())
      {
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
        {
          ...
          for (unsigned int q = 0; q < face_quadrature->size(); ++q)
          {
            local_history_max_damage_values_at_qpoints[q] = quadrature_points_history[q].max_damage;
            local_history_traction_init_values_at_qpoints0[q] = quadrature_points_history[q].traction_init[0];
            local_history_traction_init_values_at_qpoints1[q] = quadrature_points_history[q].traction_init[1];
            local_history_law_g_values_at_qpoints0[q] = quadrature_points_history[q].law_g[0];
            local_history_law_g_values_at_qpoints1[q] = quadrature_points_history[q].law_g[1];
          }

          qpoint_to_dof_matrix.vmult_add(local_history_max_damage_fe_values,
                                         local_history_max_damage_values_at_qpoints);

          qpoint_to_dof_matrix.vmult_add(local_history_traction_init_fe_values0,
                                         local_history_traction_init_values_at_qpoints0);

          qpoint_to_dof_matrix.vmult_add(local_history_traction_init_fe_values1,
                                         local_history_traction_init_values_at_qpoints1);

          qpoint_to_dof_matrix.vmult_add(local_history_law_g_fe_values0,
                                         local_history_law_g_values_at_qpoints0);

          qpoint_to_dof_matrix.vmult_add(local_history_law_g_fe_values1,
                                         local_history_law_g_values_at_qpoints1);
        }

        cell->set_dof_values(local_history_max_damage_fe_values,
                             distributed_history_max_damage_field);

        cell->set_dof_values(local_history_traction_init_fe_values0,
                             distributed_history_traction_init_field0);

        cell->set_dof_values(local_history_traction_init_fe_values1,
                             distributed_history_traction_init_field1);

        cell->set_dof_values(local_history_law_g_fe_values0,
                             distributed_history_law_g_field0);

        cell->set_dof_values(local_history_law_g_fe_values1,
                             distributed_history_law_g_field1);

        local_history_max_damage_fe_values = 0;
        local_history_traction_init_fe_values0 = 0;
        local_history_traction_init_fe_values1 = 0;
        local_history_law_g_fe_values0 = 0;
        local_history_law_g_fe_values1 = 0;
      }

    distributed_history_max_damage_field.compress(VectorOperation::insert);
    distributed_history_traction_init_field0.compress(VectorOperation::insert);
    distributed_history_traction_init_field1.compress(VectorOperation::insert);
    distributed_history_law_g_field0.compress(VectorOperation::insert);
    distributed_history_law_g_field1.compress(VectorOperation::insert);

    ghosted_history_max_damage_field = distributed_history_max_damage_field;
    ghosted_history_traction_init_field0 = distributed_history_traction_init_field0;
    ghosted_history_traction_init_field1 = distributed_history_traction_init_field1;
    ghosted_history_law_g_field0 = distributed_history_law_g_field0;
    ghosted_history_law_g_field1 = distributed_history_law_g_field1;

    DataOut<dim> data_out1;
    data_out1.add_data_vector(history_dof_handler, ghosted_history_traction_init_field1, "tr1_projected");
    data_out1.build_patches();
    data_out1.write_vtu_with_pvtu_record("./output/", "tr1_projected", cycle, mpi_communicator, 3, 0);

    parallel::distributed::SolutionTransfer<dim, LA::MPI::Vector> history_Damage_transfer(history_dof_handler);
    parallel::distributed::SolutionTransfer<dim, LA::MPI::Vector> traction_init_transfer0(history_dof_handler);
    parallel::distributed::SolutionTransfer<dim, LA::MPI::Vector> traction_init_transfer1(history_dof_handler);
    parallel::distributed::SolutionTransfer<dim, LA::MPI::Vector> law_g_transfer0(history_dof_handler);
    parallel::distributed::SolutionTransfer<dim, LA::MPI::Vector> law_g_transfer1(history_dof_handler);

    history_Damage_transfer.prepare_for_coarsening_and_refinement(distributed_history_max_damage_field);
    traction_init_transfer0.prepare_for_coarsening_and_refinement(distributed_history_traction_init_field0);
    traction_init_transfer1.prepare_for_coarsening_and_refinement(distributed_history_traction_init_field1);
    law_g_transfer0.prepare_for_coarsening_and_refinement(distributed_history_law_g_field0);
    law_g_transfer1.prepare_for_coarsening_and_refinement(distributed_history_law_g_field1);
    ...
    distributed_history_max_damage_field.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_max_damage_field.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    history_Damage_transfer.interpolate(distributed_history_max_damage_field);
    ghosted_history_max_damage_field = distributed_history_max_damage_field;

    distributed_history_traction_init_field0.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_traction_init_field0.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    traction_init_transfer0.interpolate(distributed_history_traction_init_field0);
    ghosted_history_traction_init_field0 = distributed_history_traction_init_field0;
    distributed_history_traction_init_field1.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_traction_init_field1.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    traction_init_transfer1.interpolate(distributed_history_traction_init_field1);
    ghosted_history_traction_init_field1 = distributed_history_traction_init_field1;

    distributed_history_law_g_field0.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_law_g_field0.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    law_g_transfer0.interpolate(distributed_history_law_g_field0);
    ghosted_history_law_g_field0 = distributed_history_law_g_field0;
    distributed_history_law_g_field1.reinit(history_dof_handler.locally_owned_dofs(), mpi_communicator);
    ghosted_history_law_g_field1.reinit(history_dof_handler.locally_owned_dofs(), DoFTools::extract_locally_relevant_dofs(history_dof_handler), mpi_communicator);
    law_g_transfer1.interpolate(distributed_history_law_g_field1);
    ghosted_history_law_g_field1 = distributed_history_law_g_field1;

    DataOut<dim> data_out2;
    data_out2.add_data_vector(history_dof_handler, ghosted_history_traction_init_field1, "tr1_transfered");
    ...
    std::vector<double> traction_init0_values(face_quadrature->size());
    std::vector<double> traction_init1_values(face_quadrature->size());
    std::vector<double> law_g0_values(face_quadrature->size());
    std::vector<double> law_g1_values(face_quadrature->size());
    ...
    for (const auto cell : history_dof_handler.active_cell_iterators())
      if (cell->is_locally_owned())
      {
        for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
        {
          ...
          fe_fv_damage.get_function_values(ghosted_history_max_damage_field, max_damage_values);
          fe_fv_damage.get_function_values(ghosted_history_traction_init_field0, traction_init0_values);
          fe_fv_damage.get_function_values(ghosted_history_traction_init_field1, traction_init1_values);
          fe_fv_damage.get_function_values(ghosted_history_law_g_field0, law_g0_values);
          fe_fv_damage.get_function_values(ghosted_history_law_g_field1, law_g1_values);
          ...
          for (unsigned int q = 0; q < face_quadrature->size(); ++q)
          {
            ...
            quadrature_points_history[q].traction_init[0] = traction_init0_values[q];
            quadrature_points_history[q].traction_init[1] = traction_init1_values[q];
            quadrature_points_history[q].law_g[0] = law_g0_values[q];
            quadrature_points_history[q].law_g[1] = law_g1_values[q];
  ...
  template <int dim>
  void Extrinsice_friction<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
    if (cycle != 0)
    {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::q_point_PPV(const unsigned int cycle, const unsigned int non_lin)
  {
  ...
  template <int dim>
  void Extrinsice_friction<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    TimerOutput::Scope t(computing_timer, "other");
    reaction_stress = 0.0;
  ...
  template <int dim>
  void Extrinsice_friction<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    TimerOutput::Scope t(computing_timer, "other");
  ...
  template <int dim>
  void Extrinsice_friction<dim>::run()
  {
