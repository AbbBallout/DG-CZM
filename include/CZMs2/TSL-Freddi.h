...
namespace TSL_Freddi
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("max_internal_iterations", max_internal_iterations, " ", this->prm, Patterns::Integer());
      add_parameter("geometry", geometry, "", this->prm, Patterns::Selection("reactangle|shear|peeling||hole|half_matrix|full_matrix"));
      add_parameter("TSL_law", TSL_law, "", this->prm, Patterns::Selection("linear|damage"));
    ...
    int cycles = 0, initial_refinement = 0, degree = 0, symmetry = 0, unloading = 0, reloading = 0, output_frequency = 0, max_internal_iterations = 0;
    double testing = 0, E = 0, nu = 0, Ep = 0, nup = 0, regularization = 0, n = 0, m = 0, sig_c = 0, sig_ci = 0,
           delta_c = 0, delta_ci = 0, lambda_f = 0, lambda_fi = 0, beta = 0,
    ...
    std::string quadrature = "trap", geometry = "reactangle", TSL_law = "linear";
  };
  ...
  struct PointHistory
  {
    ...
    Tensor<1, dim> cords;
    Tensor<1, dim> normals;
    Tensor<1, dim> jump;
    Tensor<1, dim> numerical_traction;
    Tensor<1, dim> analytical_traction;
    Tensor<2, dim> stress;

    Tensor<1, dim> jump_at_non_lin1;
    Tensor<1, dim> traction_at_non_lin1;
  };
  ...
  {
  public:
    TSL_Freddi(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();

  private:
    void setup_system(const bool initial_step);
    void assemble_system(const unsigned int cycle, const unsigned int non_lin, const double error, const bool has_converged);
  ...
  template <int dim>
  TSL_Freddi<dim>::TSL_Freddi(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
  ...
  template <int dim>
  void TSL_Freddi<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void TSL_Freddi<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void TSL_Freddi<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, const double error, const bool has_converged)
  {

    TimerOutput::Scope t(computing_timer, "Assemble");

    // if(non_lin==1 || has_converged)
    if (non_lin == 1)
                                 ...
                                 CopyData &copy_data)
    {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        if (non_lin == 1)
        {
          quadrature_points_history[point].jump_at_non_lin1 = g;
          quadrature_points_history[point].traction_at_non_lin1 = tr;
        }
          ...
          if (quadrature_points_history[point].is_reunload)
          {
            if (par.is_implicit)
              ...
              if (delta_c < quadrature_points_history[point].max_seperation)
                TCZ_res = sig_c * std::pow((lambda_f - quadrature_points_history[point].max_seperation) / (lambda_f - delta_c), par.m) * g_eff / quadrature_points_history[point].max_seperation;
              else
                TCZ_res = sig_c * g_eff / delta_c;
            else
            // explicit
            {
              TCZ_res = g_eff * (sig_c / std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) * std::pow((lambda_f - std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) / lambda_f, par.m);
            }
            ...
            if (par.is_implicit)
            { // implicit
              TCZ = TCZ * (sig_c / quadrature_points_history[point].max_seperation) * std::pow((lambda_f - quadrature_points_history[point].max_seperation) / (lambda_f - delta_c), par.m);
              if (delta_c > quadrature_points_history[point].max_seperation)
              {
          ...
          else
          {
            ...
            { // implcit
              if (delta_c < seperation)
                TCZ_res = sig_c * std::pow((lambda_f - seperation) / (lambda_f - delta_c), par.m) * g_eff / seperation;
              else
                TCZ_res = sig_c * g_eff / delta_c;
            }
            else
            {
              // explicit
              TCZ_res = g_eff * (sig_c / std::sqrt(seperation * seperation + regularization)) * std::pow((lambda_f - std::sqrt(seperation * seperation + regularization)) / lambda_f, par.m);
            }
            if (g[1] >= 0)
            {
              ...
              if (par.is_implicit)
              {
                ...
                if (delta_c < seperation)
                {
                  ...
                  TCZ = -TCZ * ((lambda_f + seperation * (par.m - 1)) / (seperation * seperation * (lambda_f - seperation)));
                  TCZ[0][0] += 1.0;
                  TCZ[1][1] += 1.0;
                  TCZ = TCZ * (sig_c / seperation) * std::pow((lambda_f - seperation) / (lambda_f - delta_c), par.m);
                }
            ...
            if (g[1] < 0)
            {
              ...
              if (par.is_implicit == true)
              {
                if (delta_c< seperation)
                {
                  // if (std::abs(quadrature_points_history[point].traction_at_non_lin1[0]) < -0.5 * quadrature_points_history[point].traction_at_non_lin1[1])
                  // {
                  //   TCZ_res[0] += -((seperation - delta_c) / (lambda_f - delta_c)) * penalty * std::pow(-g[0], n) + penalty * std::pow(-quadrature_points_history[point].jump_at_non_lin1[0], n);
                  //   TCZ[0][0] += ((seperation - delta_c) / (lambda_f - delta_c)) * penalty * n * std::pow(-g[0], n - 1);
                  // }
                  // else
                  // {
                  //   TCZ_res[0] += -0.5 * ((seperation - delta_c) / (lambda_f - delta_c)) * (std::abs(g[0]) / g[0]) * tr[1];
                  // }
                }
              }
              else
              {
                // if (std::abs(quadrature_points_history[point].traction_at_non_lin1[0]) < -0.5 * quadrature_points_history[point].traction_at_non_lin1[1])
                // {
                //   TCZ_res[0] += -((seperation) / (lambda_f)) * penalty * std::pow(-g[0], n) + penalty * std::pow(-quadrature_points_history[point].jump_at_non_lin1[0], n);
                //   TCZ[0][0] += ((seperation) / (lambda_f)) * penalty * n * std::pow(-g[0], n - 1);
                // }
                // else
                // {
               //   TCZ_res[0] += -0.5 * ((seperation) / (lambda_f)) * (std::abs(g[0]) / g[0]) * tr[1];
               // }
              }
            }
          }
        ...
        if (par.TSL_law == "damage")
        {
          double kt = sig_c / delta_c, kn = sig_c / delta_c;
          if (quadrature_points_history[point].is_reunload)
          {

            double damage = lambda_f * (quadrature_points_history[point].max_seperation - delta_c) / (quadrature_points_history[point].max_seperation * (lambda_f - delta_c));
            if (par.is_implicit)
            {
              if (delta_c < quadrature_points_history[point].max_seperation)
              {
                TCZ_res[0] = kt * (1 - damage) * g_eff[0];
                TCZ_res[1] = kn * (1 - damage) * g_eff[1];
              }
              else
              {
                TCZ_res[0] = kt * g_eff[0];
                TCZ_res[1] = kn * g_eff[1];
              }
            }
            else
            {
          ...
          {
            double damage = lambda_f * (seperation - delta_c) / (seperation * (lambda_f - delta_c));
            ...
            if (par.is_implicit)
            {
              if (g[1] >= 0)
              {
                if (delta_c < seperation)
                {
                  TCZ[0][0] = 1 / seperation - 1 / (seperation - delta_c);
                  TCZ[0][1] = 1 / seperation - 1 / (seperation - delta_c);
                  TCZ[1][0] = 1 / seperation - 1 / (seperation - delta_c);
                  TCZ[1][1] = 1 / seperation - 1 / (seperation - delta_c);

                  TCZ[0][0] = TCZ[0][0] * g_eff[0] * g_eff[0] * kt * damage / seperation;
                  TCZ[0][1] = TCZ[0][1] * g_eff[0] * g_eff[1] * kn * damage / seperation;
                  TCZ[1][0] = TCZ[1][0] * g_eff[1] * g_eff[0] * kt * damage / seperation;
                  TCZ[1][1] = TCZ[1][1] * g_eff[1] * g_eff[1] * kn * damage / seperation;
            ...
            if (g[1] < 0)
            {
              ...
              //     //      if (std::abs(quadrature_points_history[point].traction_at_non_lin1[0]) < - 0.5* quadrature_points_history[point].traction_at_non_lin1[1])
              //     //      {
              //     //        TCZ_res[0] += -damage*penalty * std::pow(-g[0], n) + penalty * std::pow(-quadrature_points_history[point].jump_at_non_lin1[0], n);
              //     //        TCZ[0][0] += damage*penalty * n * std::pow(-g[0], n - 1);
              //     //      }
              //     //      else
              //     //      {
              //     TCZ_res[0] += -0.5 * damage * (std::abs(g[0]) / g[0]) * tr[1];
              //     //      }
              //   }
              // }
              // else
        ...
        if (quadrature_points_history[point].is_fully_damaged)
        {
          ...
          if (g[1] < 0)
          {
            ...
            // if (std::abs(quadrature_points_history[point].traction_at_non_lin1[0]) < -0.5 * quadrature_points_history[point].traction_at_non_lin1[1] && quadrature_points_history[point].is_moving == false)
            // {
            //   TCZ_res[0] += -penalty * std::pow(-g[0], n) + penalty * std::pow(-quadrature_points_history[point].jump_at_non_lin1[0], n);
            //   TCZ[0][0] += penalty * n * std::pow(-g[0], n - 1);
            // }
            // else
            // {
            //  quadrature_points_history[point].is_moving = true;
          //    TCZ_res[0] += -0.5 * (std::abs(g[0]) / g[0]) * tr[1];
            //}
          }
        }

        quadrature_points_history[point].cords = q_points[point];
        quadrature_points_history[point].normals = normals[point];
        quadrature_points_history[point].jump = invert(Rot) * g_eff;
        quadrature_points_history[point].numerical_traction = tr_eff;
        quadrature_points_history[point].analytical_traction = invert(Rot) * TCZ_res;
        quadrature_points_history[point].stress = old_stress_avg;
        ...
        {
          Tensor<2, dim> grads_i1, grads_i2;
          if (i < n_dofs_face / 2)
          {
            grads_i1 = fe_fv1[displacements].gradient(i, point);
            grads_i2 = fe_fv2[displacements].gradient(i, point);
          }
          else
          {
            grads_i1 = fe_fv2[displacements].gradient(i - n_dofs_face / 2, point);
            grads_i2 = fe_fv1[displacements].gradient(i - n_dofs_face / 2, point);
          }

          Tensor<2, dim> strain_i1 = 0.5 * (grads_i1 + transpose(grads_i1));
          Tensor<2, dim> strain_i2 = 0.5 * (grads_i2 + transpose(grads_i2));
          Tensor<2, dim> stress_i1 = (lambda * trace(strain_i1) * Identity + 2 * mu * strain_i1);
          Tensor<2, dim> stress_i2 = (nlambda * trace(strain_i2) * Identity + 2 * nmu * strain_i2);
          Tensor<2, dim> stress_i = stress_i1 / 2 + stress_i2 / 2;
          ...
          // stress_i=lambda*trace(straini)*Identity +2*mu*straini;

          for (unsigned int j = 0; j < n_dofs_face; ++j)
          {
            Tensor<2, dim> grads_j1, grads_j2;
            if (j < n_dofs_face / 2)
            {
              grads_j1 = fe_fv1[displacements].gradient(j, point);
              grads_j2 = fe_fv2[displacements].gradient(j, point);
            }
            else
            {
              grads_j1 = fe_fv2[displacements].gradient(j - n_dofs_face / 2, point);
              grads_j2 = fe_fv1[displacements].gradient(j - n_dofs_face / 2, point);
            }

            Tensor<2, dim> strain_j1 = 0.5 * (grads_j1 + transpose(grads_j1));
            Tensor<2, dim> strain_j2 = 0.5 * (grads_j2 + transpose(grads_j2));
            Tensor<2, dim> stress_j1 = (lambda * trace(strain_j1) * Identity + 2 * mu * strain_j1);
            Tensor<2, dim> stress_j2 = (nlambda * trace(strain_j2) * Identity + 2 * nmu * strain_j2);
            Tensor<2, dim> stress_j = stress_j1 / 2 + stress_j2 / 2;
            ...
            // stress_j=lambda*trace(strainj)*Identity +2*mu*strainj;
  ...
  template <int dim>
  void TSL_Freddi<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void TSL_Freddi<dim>::updaate_q_point_hsitory()
  {
    TimerOutput::Scope t(computing_timer, "updaate_q_point_hsitory");
  ...
  template <int dim>
  void TSL_Freddi<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
    if (cycle != 0)
    {
  ...
  template <int dim>
  void TSL_Freddi<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
  ...
  template <int dim>
  void TSL_Freddi<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    TimerOutput::Scope t(computing_timer, "other");
  ...
  template <int dim>
  void TSL_Freddi<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
  {
    TimerOutput::Scope t(computing_timer, "other");
                                 ...
                                 CopyData &copy_data)
    {
      ...
      if (cell->face(f)->user_flag_set())
        for (unsigned int point = 0; point < n_q_points; ++point)
        {
          ...
          if (cycle == 0)
          {
            q_point.open("q_points/q_point_" + std::to_string(quadrature_points_history[point].cords[0]) + "_" + std::to_string(point) + ".txt");
          ...
          {
            q_point.open("q_points/q_point_" + std::to_string(quadrature_points_history[point].cords[0]) + "_" + std::to_string(point) + ".txt", std::ios_base::app);
            q_point << disp.norm() << "\t"
                    << quadrature_points_history[point].cords[0] << "\t" << quadrature_points_history[point].cords[1] << "\t"
                    << quadrature_points_history[point].jump[0] << "\t" << quadrature_points_history[point].jump[1] << "\t" << quadrature_points_history[point].jump.norm() << "\t"
                    << quadrature_points_history[point].numerical_traction[0] << "\t" << quadrature_points_history[point].numerical_traction[1] << "\t" << quadrature_points_history[point].numerical_traction.norm() << "\t"
                    << quadrature_points_history[point].analytical_traction[0] << "\t" << quadrature_points_history[point].analytical_traction[1] << "\t" << quadrature_points_history[point].analytical_traction.norm()
                    << "\n";
            q_point.close();
          }
        }
  ...
  template <int dim>
  void TSL_Freddi<dim>::q_point_PPV(const unsigned int cycle, const unsigned int non_lin)
  {
                                 ...
                                 CopyData &copy_data)
    {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        q_point << quadrature_points_history[point].cords[0] << "," << quadrature_points_history[point].cords[1] << ","
                ...
                << quadrature_points_history[point].numerical_traction[0] << "," << quadrature_points_history[point].numerical_traction[1] << "," << quadrature_points_history[point].numerical_traction.norm() << ","
                << quadrature_points_history[point].analytical_traction[0] << "," << quadrature_points_history[point].analytical_traction[1] << "," << quadrature_points_history[point].analytical_traction.norm() << ","
                << quadrature_points_history[point].stress[0][0] << "," << quadrature_points_history[point].stress[1][0] << "," << quadrature_points_history[point].stress[1][1]
                << "\n";
        q_point.close();
      }
    };
  ...
  void TSL_Freddi<dim>::run()
  {
    ...
    for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
    {
      ...
      unsigned int max_internal_iterations = par.max_internal_iterations;
      ...
      for (unsigned int cycle = 0; cycle < par.cycles; ++cycle)
      {
        ...
        while ((error > par.error && non_lin < max_nonlin_iter) || internal_iterations <= max_internal_iterations)
        {
          ...
          assemble_system(cycle, non_lin, error, has_converged);
