#ifndef TSL_everywhere_PARALLEL_H
#define TSL_everywhere_PARALLEL_H
...
namespace TSL_everywhere
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("geometry", geometry, "", this->prm, Patterns::Selection("reactangle|sheer|beam|peeling|delamitation|signle-edge|matrix|half_matrix|hole"));
      ...
      add_parameter("sig_cp", sig_cp, " ", this->prm, Patterns::Double());
      add_parameter("sig_ci", sig_ci, " ", this->prm, Patterns::Double());
      add_parameter("delta_c", delta_c, " ", this->prm, Patterns::Double());
      add_parameter("delta_cp", delta_cp, " ", this->prm, Patterns::Double());
      add_parameter("delta_ci", delta_ci, " ", this->prm, Patterns::Double());
      add_parameter("lambda_f", lambda_f, " ", this->prm, Patterns::Double());
      add_parameter("lambda_fp", lambda_fp, " ", this->prm, Patterns::Double());
      add_parameter("lambda_fi", lambda_fi, " ", this->prm, Patterns::Double());
      ...
      add_parameter("special_quadarature_everywhere", special_quadarature_everywhere, " ", this->prm, Patterns::Bool());
    ...
    double E = 0, nu = 0, Ep = 0, nup = 0, regularization, n = 0, m = 0, sig_c = 0, sig_cp = 0, sig_ci = 0,
           delta_c = 0, delta_cp = 0, delta_ci = 0, lambda_f = 0, lambda_fp = 0, lambda_fi = 0, beta = 0,
           step_length = 0, displacementx = 0, displacementy = 0, error = 0, penalty = 0, penalty_factor = 0, viscosity = 0;
    bool is_user_penalty = 0, is_implicit = 0, is_special_BC = 0, is_TSL_everywhere = 0, special_quadarature_everywhere = 0;
  ...
  template <int dim>
  class Elasticity : public Function<dim>
  {
  public:
    Elasticity(int material_Id, const ProblemParameters<dim> &par)
    {
  ...
  template <int dim>
  double Elasticity<dim>::value(const Point<dim> &p,
  ...
  struct PointHistory
  {
    ...
    Tensor<1, dim> cords;
    Tensor<1, dim> jump;
    Tensor<1, dim> numerical_traction;
    Tensor<1, dim> analytical_traction;

    Tensor<1, dim> slope;
    Tensor<1, dim> traction_at_reunload;
    Tensor<1, dim> seperation_at_reunload;
    Tensor<1, dim> seperation_at_last_iteration;
  };
  ...
  template <int dim>
  class TSL_everywhere
  {
  public:
    TSL_everywhere(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();

  private:
    void setup_system(const bool initial_step);
    void assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error);
    void print_system(unsigned int non_lin) const;
  ...
  template <int dim>
  TSL_everywhere<dim>::TSL_everywhere(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
  ...
  template <int dim>
  void TSL_everywhere<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void TSL_everywhere<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void TSL_everywhere<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error)
  {
                                     ...
                                     CopyData &copy_data)
    {
      ...
      if (par.geometry == "reactangle" || par.geometry == "beam")
      {
      ...
      if (par.geometry == "half_matrix")
      {
        ...
        {
          pcout << "No specail BC for half matrix \n";
          exit(1);
        }
      }
                                 ...
                                 CopyData &copy_data)
    {
      ...
      std::vector<double> E_values_1(n_q_points);
      std::vector<double> lambda_values_1(n_q_points);
      std::vector<double> mu_values_1(n_q_points);

      Elasticity<dim> Elasticity_1(cell->material_id(), par);
      Lambda<dim> lambda_1(cell->material_id(), par);
      Mu<dim> mu_1(cell->material_id(), par);

      Elasticity_1.value_list(q_points, E_values_1);
      lambda_1.value_list(q_points, lambda_values_1);
      mu_1.value_list(q_points, mu_values_1);

      std::vector<double> E_values_2(n_q_points);
      std::vector<double> lambda_values_2(n_q_points);
      std::vector<double> mu_values_2(n_q_points);

      Elasticity<dim> Elasticity_2(cell->neighbor(f)->material_id(), par);
      Lambda<dim> lambda_2(cell->neighbor(f)->material_id(), par);
      Mu<dim> mu_2(cell->neighbor(f)->material_id(), par);

      Elasticity_2.value_list(q_points, E_values_2);
      lambda_2.value_list(q_points, lambda_values_2);
      mu_2.value_list(q_points, mu_values_2);
      ...
      std::vector<Tensor<2, dim>> gradu_jump(n_q_points);
      fe_iv[displacements].get_jump_in_function_gradients(ghosted_solution, gradu_jump);
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        Tensor<2, dim> grad_u1 = gradu_jump[point] / 2 + gradu_avg[point];
        Tensor<2, dim> grad_u2 = -gradu_jump[point] / 2 + gradu_avg[point];

        Tensor<2, dim> old_strain_1 = 0.5 * (grad_u1 + transpose(grad_u1));
        Tensor<2, dim> old_stress_1 = (lambda_values_1[point] * trace(old_strain_1) * Identity + 2 * mu_values_1[point] * old_strain_1);

        Tensor<2, dim> old_strain_2 = 0.5 * (grad_u2 + transpose(grad_u2));
        Tensor<2, dim> old_stress_2 = (lambda_values_2[point] * trace(old_strain_2) * Identity + 2 * mu_values_2[point] * old_strain_2);

        Tensor<2, dim> old_stress_avg = 0.5 * (old_stress_1 + old_stress_2);

        // Tensor<2, dim> old_strain_avg = 0.5 * (gradu_avg[point] + transpose(gradu_avg[point]));
        //  Tensor<2, dim> old_stress_avg = (lambda * trace(old_strain_avg) * Identity + 2 * mu * old_strain_avg);

        Tensor<1, dim> tangential = cross_product_2d(normals[point]);
        Tensor<1, dim> tr, tr_eff;
        tr[0] = scalar_product(old_stress_avg, outer_product(normals[point], tangential));
        tr[1] = scalar_product(old_stress_avg, outer_product(normals[point], normals[point]));
        tr_eff[0] = tr[0] / par.beta;
        ...
        bool check = quadrature_points_history[point].is_damaged;
        ...
        if (cell->material_id() != 1 && ncell->material_id() != 1)
          sig_c = par.sig_cp;
        if (cell->material_id() != 1 && ncell->material_id() == 1 || cell->material_id() == 1 && ncell->material_id() != 1)
          sig_c = par.sig_ci;

        if (par.is_TSL_everywhere == true)
        {
          if ((tr_eff.norm() > sig_c || par.is_implicit == true))
            quadrature_points_history[point].is_damaged = true;
        }
        else
        {
          if (cell->face(f)->user_flag_set())
            quadrature_points_history[point].is_TSL_known = true;
        }

        if (check == false && quadrature_points_history[point].is_damaged == true)
          quadrature_points_history[point].slope = tr_eff / tr_eff.norm();

        double penalty = (par.penalty_factor) * (E_values_1[point] / 2 + E_values_2[point] / 2) / (cell->measure());
        if (par.is_user_penalty)
          penalty = par.penalty;
        ...
        if (par.geometry == "matrix")
        {
          ...
          if (std::abs((cell->face(f)->center()[1] - 0.5)) > 0.4)
            quadrature_points_history[point].is_damaged = false;
        }

        if (par.geometry == "reactangle" && par.is_TSL_everywhere == true)
        {
          if (std::abs((cell->face(f)->center()[1] - 1.0)) > 0.8)
            quadrature_points_history[point].is_damaged = false;
        }

        if (quadrature_points_history[point].is_damaged == false && quadrature_points_history[point].is_TSL_known == false)
          for (unsigned int i = 0; i < n_dofs_face; ++i)
          {
            Tensor<2, dim> gradi_u1 = fe_iv[displacements].jump_in_gradients(i, point) / 2 + fe_iv[displacements].average_of_gradients(i, point);
            Tensor<2, dim> gradi_u2 = -fe_iv[displacements].jump_in_gradients(i, point) / 2 + fe_iv[displacements].average_of_gradients(i, point);

            Tensor<2, dim> straini_1 = 0.5 * (gradi_u1 + transpose(gradi_u1));
            Tensor<2, dim> straini_2 = 0.5 * (gradi_u2 + transpose(gradi_u2));

            Tensor<2, dim> stressi_1 = (lambda_values_1[point] * trace(straini_1) * Identity + 2 * mu_values_1[point] * straini_1);
            Tensor<2, dim> stressi_2 = (lambda_values_2[point] * trace(straini_2) * Identity + 2 * mu_values_2[point] * straini_2);
            ...
            {
              Tensor<2, dim> gradj_u1 = fe_iv[displacements].jump_in_gradients(j, point) / 2 + fe_iv[displacements].average_of_gradients(j, point);
              Tensor<2, dim> gradj_u2 = -fe_iv[displacements].jump_in_gradients(j, point) / 2 + fe_iv[displacements].average_of_gradients(j, point);

              Tensor<2, dim> strainj_1 = 0.5 * (gradj_u1 + transpose(gradj_u1));
              Tensor<2, dim> strainj_2 = 0.5 * (gradj_u2 + transpose(gradj_u2));

              Tensor<2, dim> stressj_1 = (lambda_values_1[point] * trace(strainj_1) * Identity + 2 * mu_values_1[point] * strainj_1);
              Tensor<2, dim> stressj_2 = (lambda_values_2[point] * trace(strainj_2) * Identity + 2 * mu_values_2[point] * strainj_2);

              copy_data_face.cell_matrix(i, j) +=
                  (-fe_iv[displacements].jump_in_values(i, point) *
                       0.5 * (stressj_1 + stressj_2) * normals[point]

                   + par.symmetry *
                         (0.5 * (stressi_1 + stressi_2)) * normals[point] *
            ...
            copy_data_face.cell_rhs(i) +=
                -(
                    ...
                    + par.symmetry * (0.5 * (stressi_1 + stressi_2)) * normals[point] *
                                     ...
                                     CopyData &copy_data)
    {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        if (quadrature_points_history[point].is_damaged == true || quadrature_points_history[point].is_TSL_known == true)
        {
          Tensor<2, dim> grad_u1 = gradu_jump[point] / 2 + gradu_avg[point];
          Tensor<2, dim> grad_u2 = -gradu_jump[point] / 2 + gradu_avg[point];
          ...
          tr[0] = scalar_product(old_stress_avg, outer_product(tangential, tangential));
          tr[1] = scalar_product(old_stress_avg, outer_product(normals[point], normals[point]));
          tr_eff[0] = tr[0] / beta;
          tr_eff[1] = std::abs(tr[1]) / 2 + tr[1] / 2;
          ...
          double sig_c = 0, delta_c = 0, lambda_f = 0;
          if (cell->material_id() == 1 && ncell->material_id() == 1)
          {
            sig_c = par.sig_c;
            delta_c = par.delta_c;
          ...
          {
            sig_c = par.sig_cp;
            delta_c = par.delta_cp;
            lambda_f = par.lambda_fp;
          }
          if (cell->material_id() != 1 && ncell->material_id() == 1 || cell->material_id() == 1 && ncell->material_id() != 1)
          {
            sig_c = par.sig_ci;
            delta_c = par.delta_ci;
            lambda_f = par.lambda_fi;
          }
        ...
        // if (error < 1e-6)
        {
          ...
          double viscosity = par.viscosity, dt = 1.0;

          if (non_lin == 1)
            quadrature_points_history[point].seperation_at_last_iteration = g_eff;
          ...
          if (par.tsl_law == "exponential")
          {
            if (quadrature_points_history[point].is_reunload)
            {
              ...
              if (par.is_implicit)
                // fenics
                TCZ_res = sig_c * std::exp(1 - quadrature_points_history[point].max_seperation / delta_c) * g_eff / delta_c + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              else
                // explicit
                TCZ_res = sig_c * std::exp(-std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization) / delta_c) * g_eff / std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization) + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
            ...
            { // we are laoding
              if (par.is_implicit)
                // fenics
                TCZ_res = sig_c * std::exp(1 - seperation / delta_c) * g_eff / delta_c + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              else

                TCZ_res = sig_c * std::exp(-std::sqrt(seperation * seperation + regularization) / delta_c) * g_eff / std::sqrt(seperation * seperation + regularization) + viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              ...
              if (g[1] >= 0)
              {
                ...
                { // explicit
                  TCZ[0][0] = 1 - (g_eff[0]) * (g_eff[0]) * (1 / (delta_c * std::sqrt((seperation * seperation + regularization))) + 1 / (seperation * seperation + regularization));
                  TCZ[0][1] = -(g_eff[1]) * (g_eff[0]) * (1 / (delta_c * std::sqrt((seperation * seperation + regularization))) + 1 / (seperation * seperation + regularization));
                  TCZ[1][0] = -(g_eff[0]) * (g_eff[1]) * (1 / (delta_c * std::sqrt((seperation * seperation + regularization))) + 1 / (seperation * seperation + regularization));
                  TCZ[1][1] = 1 - (g_eff[1]) * (g_eff[1]) * (1 / (delta_c * std::sqrt((seperation * seperation + regularization))) + 1 / (seperation * seperation + regularization));
                }
              }
              else
              {
          ...
          if (par.tsl_law == "linear")
          {
            ...
            if (quadrature_points_history[point].is_reunload)
            {
              ...
              if (par.is_implicit)
                // implcit
                if (delta_c < quadrature_points_history[point].max_seperation)
                  TCZ_res = sig_c * std::pow((lambda_f + delta_c - quadrature_points_history[point].max_seperation) / lambda_f, par.m) * g_eff / quadrature_points_history[point].max_seperation + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
                else
                  TCZ_res = sig_c * g_eff / delta_c + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              else
              // explicit
              {
                TCZ_res = g_eff * (sig_c / std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) * std::pow((lambda_f - std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) / lambda_f, par.m) + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              }
              TCZ = 0.0;
              ...
              if (par.is_implicit)
                // implicit
                TCZ = TCZ * (sig_c / quadrature_points_history[point].max_seperation) * std::pow((lambda_f + delta_c - quadrature_points_history[point].max_seperation) / lambda_f, par.m);
              // explicit
              else
              {
                TCZ = TCZ * (sig_c / std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) * std::pow((lambda_f - std::sqrt(quadrature_points_history[point].max_seperation * quadrature_points_history[point].max_seperation + regularization)) / lambda_f, par.m);
              }

              TCZ[0][0] += viscosity / dt;
              TCZ[1][1] += viscosity / dt;
            ...
            else
            {
              if (par.is_implicit)
              { // implcit
                if (delta_c < seperation)
                  TCZ_res = sig_c * std::pow((lambda_f + delta_c - seperation) / lambda_f, par.m) * g_eff / seperation + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
                else
                  TCZ_res = sig_c * g_eff / delta_c + +viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              }
              else
              {
                // explicit
                TCZ_res = g_eff * (sig_c / std::sqrt(seperation * seperation + regularization)) * std::pow((lambda_f - std::sqrt(seperation * seperation + regularization)) / lambda_f, par.m) + viscosity * (g_eff - quadrature_points_history[point].seperation_at_last_iteration) / dt;
              }
              if (g[1] >= 0)
              {
                ...
                if (par.is_implicit)
                {
                  ...
                  if (delta_c < seperation)
                  {
                    ...
                    TCZ = -TCZ * ((lambda_f + delta_c + seperation * (par.m - 1)) / (seperation * seperation * (lambda_f + delta_c - seperation)));
                    TCZ[0][0] += 1.0;
                    TCZ[1][1] += 1.0;
                    TCZ = TCZ * (sig_c / seperation) * std::pow((lambda_f + delta_c - seperation) / lambda_f, par.m);
                    TCZ[0][0] += viscosity / dt;
                    TCZ[1][1] += viscosity / dt;
                  }
                ...
                else
                {
                  ...
                  TCZ[1][0] = beta * g_eff[0] * g_eff[1];
                  TCZ[1][1] = std::pow(g_eff[1], 2);
                  TCZ = -TCZ * ((lambda_f + std::sqrt(seperation * seperation + regularization) * (par.m - 1)) / ((seperation * seperation + regularization) * (lambda_f - std::sqrt(seperation * seperation + regularization))));
                  TCZ[0][0] += 1.0;
                  TCZ[1][1] += 1.0;
                  TCZ = TCZ * (sig_c / std::sqrt(seperation * seperation + regularization)) * std::pow((lambda_f - std::sqrt(seperation * seperation + regularization)) / lambda_f, par.m);
                  TCZ[0][0] += viscosity / dt;
                  TCZ[1][1] += viscosity / dt;
                }
              }
              else
              {
                ...
                { // implicit
                  if (delta_c < seperation)
                  {
                    ...
                    TCZ[0][0] += 1.0;
                    TCZ[1][1] += 0.0;
                    TCZ = TCZ * (sig_c / (seperation + delta_c)) * std::pow((lambda_f - seperation) / lambda_f, par.m);
                    TCZ[0][0] += viscosity / dt;
                    TCZ[1][1] += viscosity / dt;
                  }
                  else
          ...
          quadrature_points_history[point].cords = q_points[point];
          quadrature_points_history[point].jump = g_eff;
          quadrature_points_history[point].numerical_traction = tr_eff;
          quadrature_points_history[point].analytical_traction = TCZ_res;


          
        
          
          // if(q_points[point][0]>0.1 && q_points[point][0]<1)
          // {
          ...
          //   pcout << "TCZ_post\t" << (transpose(Rot) * TCZ * Rot)<< "\n";
          //   pcout << "TCZ_res\t" << TCZ_res[0] << "\t" << TCZ_res[1] << "\n";
          //   pcout << "TCZ_res post \t" << transpose(Rot) * TCZ_res << "\n \n";


          //   pcout << "old_stress_1\t" << old_stress_1 << "\n";
          //   pcout << "old_stress_2\t" << old_stress_2 << "\n";
          ...
          //  Tensor<2, dim> strain_i = 0.5 * (fe_iv[displacements].average_of_gradients(i, point) + transpose(fe_iv[displacements].average_of_gradients(i, point)));
          // Tensor<2, dim> stress_i = lambda * trace(strain_i) * Identity + 2 * mu * strain_i;
            ...
            for (unsigned int j = 0; j < n_dofs_face; ++j)
            {
              ...
              else if (quadrature_points_history[point].is_TSL_known)
                copy_data_face.cell_matrix(i, j) +=
                    (-fe_iv[displacements].jump_in_values(i, point) *
                         0.5 * (stressj_1 + stressj_2) * normals[point]
...

    {
      ...
      ScratchData<dim> scratch_data_Not_TSL_Everywhere(mapping, fe, quadrature, face_quadrature);
      ScratchData<dim> scratch_data_Not_TSL_Everywhere_but_damaged_quad(mapping, fe, quadrature, *damaged_face_quadrature);
      ScratchData<dim> scratch_data_Is_TSL_Everywhere(mapping, fe, quadrature, *damaged_face_quadrature);

      CopyData copy_data;

      if (par.is_TSL_everywhere == false)
      {
        if (par.special_quadarature_everywhere == false)
          MeshWorker::mesh_loop(dof_handler.begin_active(),
                                dof_handler.end(),
                                cell_worker,
                                copier,
                                scratch_data_Not_TSL_Everywhere,
                                copy_data,
          ...
          MeshWorker::mesh_loop(dof_handler.begin_active(),
                                ...
                                cell_worker,
                                copier,
                                scratch_data_Not_TSL_Everywhere_but_damaged_quad,
                                copy_data,
                                MeshWorker::assemble_own_cells | MeshWorker::assemble_boundary_faces | MeshWorker::assemble_own_interior_faces_once | MeshWorker::assemble_ghost_faces_once,
                                boundary_worker,
                                face_worker);
        ...
        MeshWorker::mesh_loop(dof_handler.begin_active(),
                              ...
                              cell_worker,
                              copier,
                              scratch_data_Is_TSL_Everywhere,
                              copy_data,
                              MeshWorker::assemble_own_cells | MeshWorker::assemble_boundary_faces | MeshWorker::assemble_own_interior_faces_once | MeshWorker::assemble_ghost_faces_once,
                              boundary_worker,
                              face_worker);
    ...
    template <int dim>
  void TSL_everywhere<dim>::updaate_q_point_hsitory()
  {
  ...
  template <int dim>
  void TSL_everywhere<dim>::print_system(unsigned int non_lin) const
  {   
      std::ofstream matrix("system/matrix_" + std::to_string(non_lin) +".txt");
      std::ofstream rhs("system/rhs_" + std::to_string(non_lin) +".txt");
      system_matrix.print(matrix);
      system_rhs.print(rhs);
  }

  template <int dim>
  void TSL_everywhere<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void TSL_everywhere<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void TSL_everywhere<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0.0;
  ...
  template <int dim>
  void TSL_everywhere<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
    ...
    std::vector<Tensor<2, dim>> gradu_avg(n_face_q_points);
  ...
  template <int dim>
  void TSL_everywhere<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
  {
          ...
          if (cell->face(face)->user_flag_set())
          {
            ...
            for (unsigned int point = 0; point < n_face_q_points; ++point)
            {
...

              {
                ...
                if (cycle == 0)
                  q_point.open("q_points/q_point" + std::to_string(counter) + ".txt");
                else
                {
                  q_point.open("q_points/q_point" + std::to_string(counter) + ".txt", std::ios_base::app);
                  q_point << quadrature_points_history[point].cords[0] << "\t" << quadrature_points_history[point].cords[1] << "\t"
                          << quadrature_points_history[point].jump.norm() << "\t"
                          << quadrature_points_history[point].numerical_traction.norm() << "\t"
                          << quadrature_points_history[point].analytical_traction.norm()
...

              {
                ...
                if (cycle == 0)
                {
                  for (int i = 1; i < par.cycles; i++)
                    q_point.open("q_points_2/cycle_" + std::to_string(i) + ".txt");
                }
                else
                {
                  q_point.open("q_points_2/cycle_" + std::to_string(cycle) + ".txt", std::ios_base::app);
                  q_point << quadrature_points_history[point].cords[0] << "," << quadrature_points_history[point].cords[1] << ","
                          << quadrature_points_history[point].jump.norm() << ","
                          << quadrature_points_history[point].numerical_traction.norm() << ","
                          << quadrature_points_history[point].analytical_traction.norm()

                          // << quadrature_points_history[point].jump[0] << "," <<quadrature_points_history[point].jump[1] << ","
                          // << quadrature_points_history[point].numerical_traction[0] << "," << quadrature_points_history[point].numerical_traction[1] << ","
                          //  << quadrature_points_history[point].analytical_traction[0] << "," << quadrature_points_history[point].analytical_traction[1]   <<
  ...
  template <int dim>
  void TSL_everywhere<dim>::plot_over_line()
  {
    ...
    for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
    {
      ...
      if (par.geometry == "reactangle" || par.geometry == "sheer")
      {
        ...
        for (const auto &cell : triangulation.active_cell_iterators())
        {
           ...
           {
            // if (std::abs(cell->vertex(v)[0] - 0) < 0.00001 && std::abs(cell->vertex(v)[1] - 1) < 0.00001)
            // {
            //   cell->vertex(v)[1] = 1.2;
             
            // }

        //   if (std::abs(cell->vertex(v)[0] - 1) < 0.00001 && std::abs(cell->vertex(v)[1] - 1) < 0.00001)
        //     {
        //       cell->vertex(v)[1] = 1.2;
     ...
     // GridTools::distort_random(0.3, triangulation, true, 556);
      ...
      if (par.geometry == "beam")
      {
        GridIn<2> gridin;
        gridin.attach_triangulation(triangulation);
        std::ifstream f("beam.msh");
      ...
      if (par.geometry == "matrix")
      {
        ...
        gridin.read_msh(f);
        GridTools::distort_random(0.25, triangulation, true, 556);
        triangulation.refine_global(par.initial_refinement);
        std::ofstream out("grid-1.vtu");
        GridOut grid_out;
        grid_out.write_vtu(triangulation, out);
