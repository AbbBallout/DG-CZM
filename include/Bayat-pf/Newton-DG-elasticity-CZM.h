...
using namespace dealii;
  ...
  template <int dim>
    ...
    public:
        ...
        double t_0=0,step_length=0,displacementx=0,displacementy=0, small=0,m=0, error=0;
 ...
 template <int dim>
  ...
  private:
   ...
   void output_results(const unsigned int cycle,const unsigned int nonlin,const unsigned int refinement_level) const;
    ...
    void print_stuff(Tensor<1,dim> jump);

  };

  template <int dim>
  void DG_Elasticity_CZM<dim>::print_stuff(Tensor<1,dim> jump)
  {
    std::cout<<"aaaaaaa" ; 
...
const auto face_worker = [&](const auto &        cell,
      ...
      const std::vector<Tensor<1, dim>> &normals = fe_iv.get_normal_vectors();
       ...
       Identity[0][0]=1; Identity[1][1]=1; 
        ...
        for (unsigned int point = 0; point < n_q_points; ++point)
        print_stuff(old_solution_jumps[point]); 
          ...
          {   
            ...
            Tensor<2,dim> straini = 0.5*(fe_iv[displacements].average_of_gradients(i, point)+transpose(fe_iv[displacements].average_of_gradients(i, point))) ;
             ...
             {  
              ...
              // std::cout<< TCZ[0][0] << "\t" << TCZ[0][1] << "\t" << TCZ[1][0] << "\t" << TCZ[1][1]<<"\n" ;   
              ...
              TCZ[1][1]+= penalty*std::pow(2,-n)*n*std::pow(std::abs(g[1])-g[1],n)/std::abs(g[1]);
                ...
                // TCZ[1][0]= 0;
                // TCZ[1][1]= -7.5;
              // if (point_counter==point)
              // {      point_counter++ ; 
              //    std::cout<<"At qpoint = " << point << "\n" ;
              //    std::cout<<g[0]<<"\t" <<g[1]<<"\n" ; 
              // std::cout<< TCZ[0][0] << "\t" << TCZ[0][1] << "\t" << TCZ[1][0] << "\t" << TCZ[1][1]<<"\n \n" ;   
              // }
  ...
  GridTools::rotate(0 , triangulation);
   ...
   for (unsigned int cycle = 0; cycle <par.cycles; ++cycle)
     ...
     disp[1]= disp[1]-par.displacementy;
       ...
       while((error>par.error && nonlin<1000)  )
       {
        if (cycle==0 & nonlin==0)
     setup_system(true);
     else
