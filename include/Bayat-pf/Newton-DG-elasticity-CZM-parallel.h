#ifndef NEWTON_DG_ELASTICITY_CZM_CZM_PARALLLEL_H
#define NEWTON_DG_ELASTICITY_CZM_CZM_PARALLLEL_H
...
namespace DG_Elasticity_CZM_parallel
{ 
using namespace dealii;
  ...
  template <int dim>
    ...
    public:
        ...
        int cycles=0,initial_refinement=0,fracture_at =0, unlaoding_begin=0, unlaoding_end=0; 
        double para_f=0,t_0=0,step_length=0,displacementx=0,displacementy=0, small=0,m=0;
...
const auto face_worker = [&](const auto &        cell,
      ...
      const std::vector<Tensor<1, dim>> &normals = fe_iv.get_normal_vectors();
       ...
       Identity[0][0]=1; Identity[1][1]=1; 
         ...
         { 
            ...
            //  if(non_lin==1)
            //  if(quadrature_points_history[point].is_initial==false && tr_eff.norm()>t_0)
            //  {quadrature_points_history[point].is_initial=true;
            //  quadrature_points_history[point].initial_cycle=cycle;
            //  }
             
          
            // if(quadrature_points_history[point].is_initial==true && quadrature_points_history[point].initial_cycle!=cycle )
            // quadrature_points_history[point].is_initial=false;


             if(tr_eff.norm()>t_0)
               quadrature_points_history[point].is_damaged=true ;

            //const double rotation = std::acos(normals[point][1]); // dot product of vector [0,1] and n
             const double normal = normals[point][1];
             const int sign_normalx = ((normals[point][0] > 0) ? 1 : ((normals[point][0]< 0) ? -1 : 0));

             R[0][0]= normal; R[0][1]=-sign_normalx*std::sqrt(1-normal*normal);
             R[1][0]=sign_normalx*std::sqrt(1-normal*normal); R[1][1]=normal;
            ...
            if(non_lin==1)
            {quadrature_points_history[point].max_seperation=std::max(quadrature_points_history[point].max_seperation,seperation);
              if(std::abs(quadrature_points_history[point].max_seperation -seperation)<1e-14)
              quadrature_points_history[point].is_unloading=false;
              else
              quadrature_points_history[point].is_unloading=true ;
            }


             if (seperation<small && non_lin<25)
             {
                  if(std::abs(g[1])<small)
                  if(g[1]>=0)
                  g[1]+=small;
                  //  else
                  //  g[1]=0;

               if(std::abs(g[0])<small)
               if(g[0]>=0)
               g[0]+=small;
               else
               g[0]=g[0]-small;
            ...
            Tensor<2,dim> straini = 0.5*(fe_iv[displacements].average_of_gradients(i, point)+transpose(fe_iv[displacements].average_of_gradients(i, point))) ;
             ...
             {  
              ...
              TCZ[1][1]+= penalty*std::pow(2,-n)*n*std::pow(std::abs(g[1])-g[1],n)/std::abs(g[1]);
                ...
                if(seperation<lambda_f && quadrature_points_history[point].is_fully_damaged == false) 
                 if(quadrature_points_history[point].is_unloading==false)
                   copy_data_face.cell_matrix(i, j) +=
                   ( fe_iv[displacements].jump_in_values(i, point))*(TCZ*fe_iv[displacements].jump_in_values(j, point))*JxW[point] ; 
                   else
                   { seperation=quadrature_points_history[point].max_seperation ;
                    TCZ=0.0;
                    TCZ[0][0]+= beta;
                    TCZ[1][1]+= 0.5*(1+((g[1] > 0) ? 1 : ((g[1] < 0) ? -1 : 0)));
                      TCZ[1][1]+= penalty*std::pow(2,-n)*n*std::pow(std::abs(g[1])-g[1],n)/std::abs(g[1]);
                ...
                copy_data_face.cell_matrix(i, j)+=0.0;
                }
  ...
  template <int dim>
  double DG_Elasticity_CZM<dim>::determine_step_length() const
  {
    ...
    if(cycle%1 == 0)
    {
     ...
     reaction(ten,3);
      std::cout<<  "   reaction force = " << ten[0] << "\t" << ten[1] << "\n" ; 
        ...
        std::cout << std::endl;
         forces << disp[0] << "\t"<<disp[1] <<  "\t"  << ten[0]  << "\t" << ten[1] << "\n" ; 
     forces.close();
      }
