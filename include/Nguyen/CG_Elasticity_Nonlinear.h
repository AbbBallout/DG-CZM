#ifndef CG_NONLINEAR_H
#define CG_NONLINEAR_H

// Contnious Galerkin series code for nonlinear FEM with prm
...
namespace CG_Elasticity_Nonlinear
{
  ...
  public:
    ...
    double E = 0, nu = 0, step_length = 0, displacementx = 0, displacementy = 0, error = 0;
  };
  ...
  template <int dim>
  class CG_Elasticity_Nonlinear
  {
  public:
    CG_Elasticity_Nonlinear(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  CG_Elasticity_Nonlinear<dim>::CG_Elasticity_Nonlinear(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::assemble_system()
  {
    Tensor<2, dim> Identity;
    Identity[0][0] = 1;
    Identity[1][1] = 1;
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::solve()
  {
  ...
  template <int dim>
  double CG_Elasticity_Nonlinear<dim>::determine_step_length()
  {
    return par.step_length;
  }

  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::output_results(const unsigned int cycle, const unsigned int nonlin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0;
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
  ...
  template <int dim>
  void CG_Elasticity_Nonlinear<dim>::run()
  {
