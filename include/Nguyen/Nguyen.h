...
// DG Galerkin series code for non_linear FEM with prm

// TO DO Apply BC point wise
...
namespace Nguyen
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("damage_integration", damage_integration, " ", this->prm, Patterns::Integer());
    ...
    int cycles = 0, initial_refinement = 0, degree = 0, damage_integration, symmetry = 0, unloading, reloading;
  ...
  {
  public:
    Nguyen(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  Nguyen<dim>::Nguyen(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
        face_quadrature(degree + 1),
        face_quadrature_damage(par.damage_integration),
        mapping(),
  ...
  template <int dim>
  void Nguyen<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void Nguyen<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void Nguyen<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin)
  {
                                  ...
                                  CopyData &copy_data)
    {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        { 
          ...
          std::cout<<"is_flat " << quadrature_points_history[point].is_flat<<"\n" ;
          std::cout << "jump\t" << g[0] << "\t" << g[1] << "\n"
                    << "separation " << g.norm() << "\n";
          std::cout << "T_eff\t" << tr_eff[0] << "\t" << tr_eff[1] << "\n"
                    << "T_eff_norm " << tr_eff.norm() << "\n";
          std::cout << "traction_normed\t" << quadrature_points_history[point].normed_traction[0] << "\t" << quadrature_points_history[point].normed_traction[1] << "\n" ;
  ...
  template <int dim>
  void Nguyen<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void Nguyen<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void Nguyen<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0;
  ...
  template <int dim>
  void Nguyen<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
  ...
  template <int dim>
  void Nguyen<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
  {
    ...
    for (const auto &cell : triangulation.active_cell_iterators())
      for (const unsigned int face : cell->face_indices())
        if (std::abs(cell->face(face)->center()[1] - 1) < 1e-5)
          cell->face(face)->set_user_flag();
        else
          cell->face(face)->clear_user_flag();
  }
  ...
  template <int dim>
  void Nguyen<dim>::plot_over_line()
  {
  ...
  void Nguyen<dim>::run()
  {
    ...
    for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
    {
      ...
      for (unsigned int cycle = 0; cycle < par.cycles; ++cycle)
      {
        ...
        while ((error > par.error) && non_lin < 100)
        { 
