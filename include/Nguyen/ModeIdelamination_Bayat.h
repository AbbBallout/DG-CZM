#ifndef MODEIDELAMETATION_BAYAT_H
#define MODEIDELAMETATION_BAYAT_H
...
namespace BayatIDela
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("damage_qaud", damage_qaud, " ", this->prm, Patterns::Integer());
    ...
    int cycles = 0, initial_refinement = 0, degree = 0, damage_qaud=0, symmetry = 0,unloading, reloading;
    double E = 0, nu = 0, step_length = 0, displacementx = 0, displacementy = 0, error = 0, penalty = 0, t_0 = 0, lambda_f = 0, m = 0;
  ...
  {
  public:
    BayatIDela(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  private:
    ...
    const QGauss<dim - 1> face_quadrature_damage;
  ...
  template <int dim>
  BayatIDela<dim>::BayatIDela(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
        face_quadrature(degree+1),
        face_quadrature_damage(par.damage_qaud),
        mapping(),
  ...
  template <int dim>
  void BayatIDela<dim>::setup_quadrature_point_history()
  {
    ...
    quadrature_point_history.resize(triangulation.n_active_faces() * face_quadrature_damage.size());

    unsigned int history_index = 0;
    for (auto &face : triangulation.active_face_iterators())
    {
      face->set_user_pointer(&quadrature_point_history[history_index]);
      history_index += face_quadrature_damage.size();
    }
  ...
  template <int dim>
  void BayatIDela<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void BayatIDela<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin)
  {
                                     ...
                                     CopyData &copy_data)
    {
      ...
      if (std::fabs(cell->face(face_no)->center()[0]) > 50-5*std::pow(2,-(float)par.initial_refinement)-1e-10)
     for (unsigned int point = 0; point < n_q_points; ++point)
            {
              double penalty = (1e+3) * ((2 * mu_values[point]) * (3 * lambda_values[point] + 2 * mu_values[point]) / (lambda_values[point] + mu_values[point])) / (cell->measure());
              if (par.is_user_penalty)
...

  {
    ...
    { 
      ...
      PointHistory<dim> *quadrature_points_history = reinterpret_cast<PointHistory<dim> *>(cell->face(f)->user_pointer());
      double t_0 = par.t_0, lambda_f = par.lambda_f;
      Tensor<1, dim> g, g_eff;
      Tensor<1, dim> tr, tr_eff; // traction
      Tensor<1, dim> tangential;
      Tensor<2, dim> TCZ;
   ...
   if (cell->face(f)->user_flag_set())
     if (cell->face(f)->center()[0]>40)
        ...
        if (cell->face(f)->user_flag_set())
          if (tr_eff.norm() > t_0)
            quadrature_points_history[point].is_flat = true;
          else
            quadrature_points_history[point].is_flat = false;

        if (cell->face(f)->user_flag_set())
          if (tr_eff.norm() > t_0 - 0.1 * t_0)
            check = true;
        ...
        if (cell->face(f)->user_flag_set())
          if (tr_eff.norm() < t_0 - 0.1 * t_0 && check == false)
            is_relaxed = false;

        quadrature_points_history[point].normed_traction = tr_eff / tr_eff.norm();
        ...
        if (std::abs(quadrature_points_history[point].max_seperation - seperation) > 1e-5)
        ...
        if (quadrature_points_history[point].is_flat)
        {
          TCZ[0][0] = 0.0;
          TCZ[0][1] = 0.0;
          TCZ[1][0] = 0.0;
          TCZ[1][1] = 0.0;
          //  TCZ_res = par.t_0*quadrature_points_history[point].normed_traction;

          TCZ_res = par.t_0 * quadrature_points_history[point].normed_traction * std::pow((lambda_f - seperation) / lambda_f, m);
        }

        else if (quadrature_points_history[point].is_fully_damaged)
        {
          TCZ = 0;
          TCZ_res = 0;
        }
        else if ( false && quadrature_points_history[point].is_reunload)
        {
          TCZ = 0;
          TCZ_res = 0;
          ...
          TCZ = TCZ * (t_0 / quadrature_points_history[point].max_seperation) * std::pow((lambda_f - quadrature_points_history[point].max_seperation) / lambda_f, m);

          TCZ[1][1] += penalty * n * std::pow(std::abs(g[1]) / 2 - g[1] / 2, n - 1);

          TCZ_res = g_eff * (t_0 / quadrature_points_history[point].max_seperation) * std::pow((lambda_f - quadrature_points_history[point].max_seperation) / lambda_f, m);
        ...
        if (quadrature_points_history[point].is_damaged && quadrature_points_history[point].is_fully_damaged==false )
        { 
...

    {
      ...
      ScratchData<dim> scratch_data(mapping, fe, quadrature, face_quadrature_damage);
  ...
  template <int dim>
  void BayatIDela<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void BayatIDela<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void BayatIDela<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0;
  ...
  template <int dim>
  void BayatIDela<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
    ...
    for (const auto &cell : triangulation.active_cell_iterators())
      for (const unsigned int face : cell->face_indices())
            if (std::abs(cell->face(face)->center()[1] - 2.5) < 1e-5)
...
template <int dim>
void BayatIDela<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
{
    ...
    const unsigned int n_face_q_points = face_quadrature_damage.size();
 ...
 template <int dim>
  void BayatIDela<dim>::plot_over_line()
  {
  ...
  void BayatIDela<dim>::run()
  {
    ...
    for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
    {
      triangulation.clear();
      // Mesh geenration
      const Point<dim> P1(35,0), P2(50, 5);
      const std::vector<unsigned int> repetitions{3, 2};
  ...
  //  {   
    ...
    //  }
     ...
     disp[1]= disp[1]-par.displacementy;
        ...
        while (error > par.error && non_lin < 50)
        {
