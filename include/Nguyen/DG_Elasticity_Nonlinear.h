#ifndef DG_ELASTICITY_NONLINEAR_H
#define DG_ELASTICITY_NONLINEAR_H

// DG Galerkin series code for nonlinear FEM with prm
...
namespace DG_Elasticity_Nonlinear
{
  ...
  public:
    ...
    double E = 0, nu = 0, step_length = 0, displacementx = 0, displacementy = 0, error = 0, penalty = 0;
  ...
  template <int dim>
  class DG_Elasticity_Nonlinear
  {
  public:
    DG_Elasticity_Nonlinear(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  DG_Elasticity_Nonlinear<dim>::DG_Elasticity_Nonlinear(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::assemble_system()
  {
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::solve()
  {
  ...
  template <int dim>
  double DG_Elasticity_Nonlinear<dim>::determine_step_length()
  {
    return par.step_length;
  }

  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::output_results(const unsigned int cycle, const unsigned int nonlin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0;
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
  ...
  template <int dim>
  void DG_Elasticity_Nonlinear<dim>::run()
  {
    ...
    for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
    {
      ...
      for (unsigned int cycle = 0; cycle < par.cycles; ++cycle)
      {
        ...
        while (error > par.error && nonlin < 10)
        {
          if (cycle == 0 && nonlin == 0)
            setup_system(true);
          else
