#ifndef PAHSEFILEDDGSQ_H
#define PAHSEFILEDDGSQ_H
...
{
public:
    PF_DG_sq(const unsigned int degree);
    void run();
...
private:
  ...
  template <int dim>
  void PF_DG_sq<dim>::setup_quadrature_point_history()
  {
...
template <int dim>
PF_DG_sq<dim>::PF_DG_sq(const unsigned int degree)
    : degree(degree), 
    quadrature(degree + 1), 
  ...
  void PF_DG_sq<dim>::import_mesh()
{
  ...
  template <int dim>
  void PF_DG_sq<dim>::setup_system(const bool initial_step)
  {
 ...
 template <int dim>
 void PF_DG_sq<dim>::assemble_system()
 {
...
const auto face_worker = [&](const auto &        cell,
    ...
    const double penalty = 1e+5*get_penalty_factor(degree, extent1, extent1);
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
              {   
                ...
                + (2*mu_values[point]+3*lambda_values[point])* penalty*                 
                   ...
                   * (cell->face(face_no)->boundary_id()-0)*disp *
               JxW[point]; // dx
  ...
  template <int dim>
  void PF_DG_sq<dim>::solve_time_step()
  { 
  ...
  template <int dim>
  void PF_DG_sq<dim>::output_results(const unsigned int cycle,const unsigned int refinement_level) const
  {
  ...
  data_out.write_vtu(output);
      ...
      template <int dim>
    void PF_DG_sq<dim>::output_strain(const unsigned int cycle,const unsigned int refinement_level) const
    { 
    ...
    template <int dim>
    void PF_DG_sq<dim>::output_stress(const unsigned int cycle,const unsigned int refinement_level) const
    {
 ...
 void PF_DG_sq<dim>::move_mesh()
{
 ...
 template <int dim>
  void PF_DG_sq<dim>::reaction (Tensor<1,dim> &reaction_stress, const types::boundary_id &boundary_id)
{       
        reaction_stress = 0; 
 ...
 template <int dim>
 void PF_DG_sq<dim>::commulative_opening (double &opening, const bool &interface_or_bulk)
{       
        opening = 0; 
...
{   
   ...
   // flag the material interface edges. Weird that this should be done after refinement 
    ...
    for (unsigned int cycle = 0; cycle <250; ++cycle)
    { 
