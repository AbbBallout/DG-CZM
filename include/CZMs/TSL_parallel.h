#ifndef TSL_prllel_PARALLEL_H
#define TSL_prllel_PARALLEL_H
...
namespace TSL_prllel
{
  ...
  {
  public:
    TSL_prllel(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  TSL_prllel<dim>::TSL_prllel(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
  ...
  template <int dim>
  void TSL_prllel<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void TSL_prllel<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void TSL_prllel<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error)
  {
                                     ...
                                     CopyData &copy_data)
    {
      ...
      if (par.geometry == "reactangle")
      {
        if (par.is_special_BC == false)
        {
          ...
          if ((cell->face(face_no)->boundary_id() == 2) || cell->face(face_no)->boundary_id() == 3 && is_at_corner)
            for (unsigned int point = 0; point < n_q_points; ++point)
            {
                                 ...
                                 CopyData &copy_data)
    {
      ...
      if (cell->face(f)->user_flag_set() == false)
        for (unsigned int point = 0; point < n_q_points; ++point)
        {
          Tensor<2, dim> old_strain = 0.5 * (gradu_avg[point] + transpose(gradu_avg[point]));
          Tensor<2, dim> old_stress = (lambda_values[point] * trace(old_strain) * Identity + 2 * mu_values[point] * old_strain);
  ...
  template <int dim>
  void TSL_prllel<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void TSL_prllel<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void TSL_prllel<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0.0;
  ...
  template <int dim>
  void TSL_prllel<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
  ...
  template <int dim>
  void TSL_prllel<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
  {
  ...
  template <int dim>
  void TSL_prllel<dim>::plot_over_line()
  {
