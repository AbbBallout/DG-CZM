#ifndef Exponential_TSL_H
#define Exponential_TSL_H

// Fenoic exaple but with newton rhapson. It deosn' work.
...
namespace Exponential_TSL
{
  ...
  public:
    ...
    double E = 0, nu = 0, n=0, sig_c = 0, delta_c = 0, step_length = 0, displacementx = 0, displacementy = 0, error = 0, penalty = 0, penalty_factor = 0;
  ...
  template <int dim>
  class Exponential_TSL
  {
  public:
    Exponential_TSL(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();

  private:
    void setup_system(const bool initial_step);
    void assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error);
    void assemble_system_fixed_point();
    ...
    const QGaussLobatto<dim - 1> damaged_face_quadrature;
  ...
  template <int dim>
  Exponential_TSL<dim>::Exponential_TSL(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
        face_quadrature(degree+1),
        damaged_face_quadrature(degree+1),
        mapping(),
  ...
  template <int dim>
  void Exponential_TSL<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void Exponential_TSL<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void Exponential_TSL<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error)
  {
  ...
  template <int dim>
  void Exponential_TSL<dim>::assemble_system_fixed_point()
  {
                                 ...
                                 CopyData &copy_data)
    {
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        TCZ_res = sig_c * std::exp(1 - old_solution_jumps[point].norm() / delta_c) * old_solution_jumps[point] / delta_c;

        law = sig_c * std::exp(1 - old_solution_jumps[point].norm() / delta_c) / delta_c;
        ...
        for (unsigned int i = 0; i < n_dofs_face; ++i)
        {
          ...
          for (unsigned int j = 0; j < n_dofs_face; ++j)
          {
            Tensor<2, dim> strainj = 0.5 * (fe_iv[displacements].average_of_gradients(j, point) + transpose(fe_iv[displacements].average_of_gradients(j, point)));
            if (cell->face(f)->user_flag_set() == true)
              copy_data_face.cell_matrix(i, j) += fe_iv[displacements].jump_in_values(i, point) * law * fe_iv[displacements].jump_in_values(j, point) * JxW[point];
            else
              copy_data_face.cell_matrix(i, j) +=
                  (-fe_iv[displacements].jump_in_values(i, point) *
                       (lambda_values[point] * trace(strainj) * Identity + 2 * mu_values[point] * strainj) * normals[point]
  ...
  template <int dim>
  void Exponential_TSL<dim>::solve(const unsigned int cycle)
  {
  ...
  template <int dim>
  void Exponential_TSL<dim>::solve_fixed_point()
  {
    ...
    SparseDirectUMFPACK A_direct;
    A_direct.initialize(system_matrix);
    A_direct.vmult(current_solution, system_rhs);
  }

  template <int dim>
  void Exponential_TSL<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
  {
  ...
  template <int dim>
  void Exponential_TSL<dim>::reaction(Tensor<1, dim> &reaction_stress, const types::boundary_id &boundary_id)
  {
    reaction_stress = 0;
  ...
  template <int dim>
  void Exponential_TSL<dim>::interface_jump_and_traction(Tensor<1, dim> &interface_jump, Tensor<1, dim> &interface_stress)
  {
    interface_stress = 0;
    interface_jump = 0;
  ...
  template <int dim>
  void Exponential_TSL<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
  {
        ...
        if (cell->face(face)->user_flag_set())
        {
          ...
          for (unsigned int point = 0; point < n_face_q_points; ++point)
          {
            ...
            if (cycle == 0)
            {
              ...
              {
                std::cout << "q_points folder doesn't exist \n";
                exit(1);
              }
            }
            else
  ...
  template <int dim>
  void Exponential_TSL<dim>::plot_over_line()
  {
