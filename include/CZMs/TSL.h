...
namespace TSL
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("geometry", geometry, "", this->prm, Patterns::Selection("reactangle|peeling||delamitation|matrix"));
  ...
  {
  public:
    TSL(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  TSL<dim>::TSL(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
  ...
  template <int dim>
  void TSL<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void TSL<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void TSL<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error)
  {
  ...
  if ( cell->face(face_no)->boundary_id() == 2 && is_at_corner)
      ...
      if (par.geometry == "matrix")
      {
        if(par.is_special_BC==false)
        {
          ;//You have to rotate with R now that the mesh isn't perfectly square
        }
        else
        {
          std::cout<<"no specail BC set for matrix geometry \n" ;
          exit(1);
        }
      }
    };
                                 ...
                                 CopyData &copy_data)
    {
      ...
      if (cell->face(f)->user_flag_set() == false)
        for (unsigned int point = 0; point < n_q_points; ++point)
        {
          ...
          Tensor<2, dim> old_stress = (lambda_values * trace(old_strain) * Identity + 2 * mu_values * old_strain);

          double penalty = (par.penalty_factor) * ((2 * mu_values) * (3 * lambda_values + 2 * mu_values) / (lambda_values + mu_values)) / (cell->measure());
          if (par.is_user_penalty)
            penalty = par.penalty;
          ...
          for (unsigned int i = 0; i < n_dofs_face; ++i)
          {
            ...
            for (unsigned int j = 0; j < n_dofs_face; ++j)
            {
              ...
              copy_data_face.cell_matrix(i, j) +=
                  (-fe_iv[displacements].jump_in_values(i, point) *
                       (lambda_values * trace(strainj) * Identity + 2 * mu_values * strainj) * normals[point]

                   + par.symmetry *
                         (lambda_values * trace(straini) * Identity + 2 * mu_values * straini) * normals[point] *
            ...
            copy_data_face.cell_rhs(i) +=
                -(
                    ...
                    + par.symmetry * (lambda_values * trace(straini) * Identity + 2 * mu_values * straini) * normals[point] *
              ...
              else
              {
                ...
                else
                {
                  ...
                  { // implicit
                    if (quadrature_points_history[point].is_damaged)
                    {
                      TCZ[0][0] = std::pow(beta, 3) * g_eff[0] * (g_eff[0] + par.delta_c * quadrature_points_history[point].slope[0]);
                      TCZ[0][1] = 0.0;
                      TCZ[1][0] = 0.0;
                      TCZ[1][1] = 0.0;
                      TCZ = -TCZ * ((par.lambda_f + par.delta_c + seperation * (par.m - 1)) / (seperation * seperation * (par.lambda_f + par.delta_c - seperation)));
    ...
    template <int dim>
    void TSL<dim>::solve(const unsigned int cycle)
    {
    ...
    template <int dim>
    void TSL<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
    {
    ...
    template <int dim>
    void TSL<dim>::reaction(Tensor<1, dim> & reaction_stress, const types::boundary_id &boundary_id)
    {
      reaction_stress = 0.0;
    ...
    template <int dim>
    void TSL<dim>::interface_jump_and_traction(Tensor<1, dim> & interface_jump, Tensor<1, dim> & interface_stress)
    {
      interface_stress = 0;
      interface_jump = 0;
    ...
    template <int dim>
    void TSL<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
    {
    ...
    void TSL<dim>::run()
    {
      ...
      for (unsigned int refinement_level = 0; refinement_level < 1; refinement_level++)
      {
        ...
        for (unsigned int cycle = 0; cycle < par.cycles; ++cycle)
        {
          ...
          while (error > par.error && error2 > par.error && non_lin < 50)
          {
