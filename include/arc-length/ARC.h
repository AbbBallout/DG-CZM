...
namespace ARC
{
  ...
  public:
    ProblemParameters() : ParameterAcceptor("main")
    {
      ...
      add_parameter("geometry", geometry, "", this->prm, Patterns::Selection("reactangle"));
  ...
  {
  public:
    ARC(const unsigned int degree, const ProblemParameters<dim> &par);
    void run();
  ...
  template <int dim>
  ARC<dim>::ARC(const unsigned int degree, const ProblemParameters<dim> &par)
      : par(par),
        degree(degree),
        quadrature(degree + 1),
  ...
  template <int dim>
  void ARC<dim>::setup_quadrature_point_history()
  {
  ...
  template <int dim>
  void ARC<dim>::setup_system(const bool initial_step)
  {
  ...
  template <int dim>
  void ARC<dim>::assemble_system(const unsigned int cycle, const unsigned int non_lin, double &error)
  {
    ...
    template <int dim>
    void ARC<dim>::solve(const unsigned int cycle)
    {
    ...
    template <int dim>
    void ARC<dim>::output_results(const unsigned int cycle, const unsigned int non_lin, const unsigned int refinement_level) const
    {
    ...
    template <int dim>
    void ARC<dim>::reaction(Tensor<1, dim> & reaction_stress, const types::boundary_id &boundary_id)
    {
      reaction_stress = 0.0;
    ...
    template <int dim>
    void ARC<dim>::interface_jump_and_traction(Tensor<1, dim> & interface_jump, Tensor<1, dim> & interface_stress)
    {
      interface_stress = 0;
      interface_jump = 0;
    ...
    template <int dim>
    void ARC<dim>::interface_jump_and_traction_at_qpoints(const unsigned int cycle)
    {
