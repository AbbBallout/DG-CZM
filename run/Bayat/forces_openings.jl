...
# normal plot 
f= readdlm("/media/abbas/Ubuntu1/MainUbu/PhD-Parma/Solvers/Phase-field-Lornenzo-extended/run/Bayat/forces0.txt")
#plot(f[:,1],f[:,3], label = ["force_x"])
#plot(f[:,2],f[:,4],label = ["force_y"])
plot(sqrt.(f[:,1].*f[:,1] +f[:,2].*f[:,2]),sqrt.(f[:,3].*f[:,3]+f[:,4].*f[:,4]),title = "t> to solution, tol xxx, dsip+=0.00xx, \n mesh X, XXX dofs" ,label = ["force_mag"]) 
plot!(sqrt.(f[:,5].*f[:,5] +f[:,6].*f[:,6]),sqrt.(f[:,7].*f[:,7]+f[:,8].*f[:,8])) 

plot!(sqrt.(f[:,1].*f[:,1] +f[:,2].*f[:,2]),sqrt.(f[:,3].*f[:,3]+f[:,4].*f[:,4]), seriestype=:scatter,markersize=:1 ) 
